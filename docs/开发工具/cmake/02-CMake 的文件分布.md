...
其中第一种方案会在[【08】find_package 详解](08-find_package%20详解.md)中介绍。
...
`*.in` 模板文件一般是用于在 cmake 执行阶段（准确来说，应该是配置阶段）将其嵌入的 CMake 变量、列表展开，并生成目标文件的过程中的。这一步通常是由 `configure_file()` 完成，会在[【06】变量参与 C++ 的编译](06-变量参与%20C++%20的编译.md) 中进行介绍。`*.in` 的后缀是约定俗成的，当然也可以使用其他形式的后缀。
